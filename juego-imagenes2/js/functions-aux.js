  ...
  if (count >= 2) {
    ...
    if (con == true) {
      ...
      for(let j=0; j<columnas; j++){
        let text = document.getElementById("text");
        if(mAux[i][j]!==0){
          text.innerHTML +='<img src="../img/img' + mAux[i][j] + '.jpg" alt="">';
        }else if(mAux[i][j]==0){
          text.innerHTML +=
          '<img src="../img/buscame.jpg" onclick="img(' + i +"," + j +"," + mAux[i][j] + ')" alt="">';
        }
      }
      }
    } else {
  ...
  for (let r = 0; r < filas; r++) {
    for (let c = 0; c < columnas; c++) {
      if (r === i && c === j) {
        ...
        text.innerHTML +=
          '<img src="../img/buscame.jpg" onclick="img(' + r +"," + c +"," + matriz[r][c] +')" alt="">';
      }
